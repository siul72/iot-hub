...
import co.luism.diagnostics.common.XmlList;

import co.luism.datacollector.*;
...
import co.luism.datacollector.common.DataCollectorImportUtils;
...
public class WebManagerFacade implements IWebManagerFacade {
    ...
    public ReturnCode validateUser(String login, String password) {
        ...
        User u = WebManager.getInstance().getUser(login);
    ...
    public List<Organization> getOrganizationList() {
        ...
        myOrgList.addAll(WebManager.getInstance().getOrganizationMap().values());
        return myOrgList;
    }
    ...
    public List<DataTag> getAllTags() {
        ...
        return WebManager.getInstance().getAllTags();
    }
    ...
    public boolean createData(DiagnosticsPersistent bean) {
        ...
        if(bean.create()){
            if (bean instanceof Organization){
                WebManager.getInstance().add((Organization) bean);
            }
            ...
            if (bean instanceof User){
                ...
                WebManager.getInstance().add((User) bean);
            }

            if (bean instanceof Fleet){
                WebManager.getInstance().add((Fleet) bean);
            }

            if (bean instanceof DataTag){
                WebManager.getInstance().add((DataTag) bean);
            }

            if (bean instanceof Vehicle){
                WebManager.getInstance().add((Vehicle) bean);
            }
    ...
    public boolean reloadOrganization(){
        WebManager.getInstance().reloadOrganization();
        return true;
    }

    @Override
    public boolean restartWebManager(Class clazz){
        WebManager.getInstance().restartWebManager(clazz);
        return true;
    }
    ...
    public boolean saveEventConfigurationToDataBase(File odsCognitioFileName) {
        ...
        WebManager.getInstance().loadEventConfigurationToDatabase(cnf);
        return true;
    }
    ...
    public boolean saveProcessConfigurationToDataBase(File odsProcessFileName, Integer configurationId) {
        ...
        WebManager.getInstance().loadProcessDataConfigurationToDatabase(mySet, configurationId);
        return true;
    }
    ...
    public boolean saveCategoriesConfigurationToDataBase(File odsCognitioFileName) {
        ...
        Configuration myConf = WebManager.getInstance().getConfiguration(cnf);

        WebManager.getInstance().saveCategories(myConf, cnf);
        return true;
    }

    @Override
    public void reloadDataTags() {
        WebManager.getInstance().reloadDataTags();
    }
    ...
    public void addListener(IDiagnosticsEventHandler listener) {
        WebManager.getInstance().addListener(listener);
    }

    @Override
    public void removeListener(IDiagnosticsEventHandler listener) {
        WebManager.getInstance().removeListener(listener);
    }
    ...
    public DataTag getAlarmTagById(Integer id) {
        return WebManager.getInstance().getAlarmTagById(id);
    }

    @Override
    public DataTag getTagBySourceId(EventTypeEnum t, int configurationId, int sourceId) {
        return WebManager.getInstance().getTagBySourceTagId(t, configurationId, sourceId);
    }

    @Override
    public List<AlarmBuffer> getAllBuffers() {
        List<AlarmBuffer> myList = new ArrayList<>();
        myList.addAll(WebManager.getInstance().getBufferSet());
    ...
    public List<AlarmCategory> getAllCategories() {
        ...
        myList.addAll(WebManager.getInstance().getCategorySet());
    ...
    public Vehicle getVehicle(String vid) {
        return WebManager.getInstance().findVehicle(vid);
    }

    @Override
    public DataTag getTagByName(EventTypeEnum tagDataTypeSystem, int configurationId, String name) {
        return WebManager.getInstance().getTagByName(tagDataTypeSystem, configurationId, name);
    }

    @Override
    public DataTag getTagById(Integer id) {
        return WebManager.getInstance().getTagById(id);
    }
    ...
    public AlarmCategory getCategory(Integer categoryId) {
        ...
        return WebManager.getInstance().getCategory(categoryId);

    }

    @Override
    public DataTag getTagByProcessId(Integer processId){
        return WebManager.getInstance().getTagByProcessId(processId);
    }
